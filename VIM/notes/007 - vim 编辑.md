...
| 按键     | 说明                                                                                                                                                    | 备注                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `y`      | 按下`→`，向右复制一个字符<br/>按下`←`，向左复制一个字符<br/>按下`w`，向右复制一个单词<br/> 按下`e`，向右复制到当前单词末<br/> 按下`b`，向左复制一个单词 | copy char forward<br/>copy char backward<br/>copy word forward，包括空格<br/> copy word forward，不包括空格<br/> copy word backward |
...
- 实际上这里的删除都是 `delete [into register x]`，等效于剪切，可使用 `p`/`P` 粘贴回来。  
...
3. 按下大写的 <kbd>I</kbd>（`<S-i>`）进入插入模式，输入注释符（`//` 或 `#`），然后连按两下 <kbd>Esc</kbd> 列插入，返回整行模式。  
...
如果上一步的注释是双字符 `//`，则第2步需先左右选中2列，再竖选行，然后按下  <kbd>d</kbd> 键删除列选。
...
`:m,nw >> <file>`: 将 m 行到 n 行的内容追加到文件 <file> 末尾  
